...
void RaftNodeManager::refresh_nodes_dns_native(const std::string& nodes_config, bool force_dns_refresh) {
    LOG(INFO) << "Refreshing nodes with DNS-native support, force_refresh=" << force_dns_refresh;

    // Parse configuration with hostname/IP classification
    raft_config::NodeConfiguration node_config = raft_config::parse_node_configuration(nodes_config);

    // Convert to braft configuration with fresh DNS resolution
    braft::Configuration new_conf = raft_config::node_config_to_braft(node_config);

    if (new_conf.empty()) {
        LOG(ERROR) << "Failed to parse nodes configuration: " << nodes_config;
        return;
    }

    // Record configuration change in history
    {
        std::lock_guard<std::mutex> lock(config_history_mutex);
        config_history.push_back(nodes_config);
        // Keep only last 10 entries
        if (config_history.size() > 10) {
            config_history.erase(config_history.begin());
        }
    }
    ...
    std::unique_lock lock(node_mutex);
    if (node) {
        RefreshNodesClosure* closure = new RefreshNodesClosure();
        node->change_peers(new_conf, closure);
        LOG(INFO) << "DNS-native node refresh initiated";
    }
}

nlohmann::json RaftNodeManager::get_comprehensive_status() const {
    nlohmann::json status = get_status(); // Start with basic status

    // Add DNS-specific information
    status["dns_refresh_needed"] = dns_refresh_needed.load();
    status["last_leader_contact_ms"] = time_since_leader_contact().count();
    status["has_recent_heartbeat"] = has_recent_leader_heartbeat();

    // Add TLA+ validation information
    status["current_term"] = get_current_term();
    status["committed_index"] = get_committed_index();
    status["last_index"] = get_last_index();
    status["known_applied_index"] = get_known_applied_index();

    // Add node statistics
    status["statistics"] = get_node_statistics();

    // Add configuration history
    {
        std::lock_guard<std::mutex> lock(config_history_mutex);
        status["config_history"] = config_history;
    }

    return status;
}

nlohmann::json RaftNodeManager::get_node_statistics() const {
    nlohmann::json stats;

    // Basic operational metrics
    stats["read_ready"] = is_read_ready();
    stats["write_ready"] = is_write_ready();
    stats["leader_term"] = get_leader_term();

    // Performance metrics (would be enhanced with actual measurements)
    stats["uptime_seconds"] = 0; // Placeholder
    stats["total_operations"] = 0; // Placeholder
    stats["operations_per_second"] = 0; // Placeholder

    // Health metrics
    stats["is_healthy"] = is_read_ready() && is_write_ready();
    stats["health_score"] = has_recent_leader_heartbeat() ? 100 : 0;

    return stats;
}

bool RaftNodeManager::needs_dns_refresh() const {
    return dns_refresh_needed.load();
}

void RaftNodeManager::trigger_dns_refresh() {
    LOG(INFO) << "DNS refresh triggered manually";
    dns_refresh_needed.store(true);
}

uint64_t RaftNodeManager::get_current_term() const {
    std::shared_lock lock(node_mutex);
    if (!node) return 0;
...
int64_t RaftNodeManager::get_committed_index() const {
    std::shared_lock lock(node_mutex);
    if (!node) return -1;
...
int64_t RaftNodeManager::get_last_index() const {
    std::shared_lock lock(node_mutex);
    if (!node) return -1;
...
int64_t RaftNodeManager::get_known_applied_index() const {
    std::shared_lock lock(node_mutex);
    if (!node) return -1;
...
bool RaftNodeManager::has_recent_leader_heartbeat() const {
    auto last_contact = last_leader_contact_time.load();
    auto now = std::chrono::steady_clock::now().time_since_epoch().count();
    const auto HEARTBEAT_TIMEOUT_MS = 5000; // 5 seconds

    return (now - last_contact) < (HEARTBEAT_TIMEOUT_MS * 1000000); // Convert to nanoseconds
}

std::chrono::milliseconds RaftNodeManager::time_since_leader_contact() const {
    auto last_contact = last_leader_contact_time.load();
    auto now = std::chrono::steady_clock::now().time_since_epoch().count();
    return std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::nanoseconds(now - last_contact));
}

std::vector<std::string> RaftNodeManager::get_config_change_history() const {
    std::lock_guard<std::mutex> lock(config_history_mutex);
