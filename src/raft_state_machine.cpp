...
bool RaftStateMachine::config_is_safe() const {
...
bool RaftStateMachine::has_term_quorum_check() const {
    ...
    // Get current term from node manager
    uint64_t current_term = 0;
    // This would need to be implemented in RaftNodeManager
    // For now, we'll use a simplified check

    uint64_t last_check_term = last_term_quorum_check.load();

    // Check if we've committed at least one operation in the current term
    if (current_term > 0 && current_term > last_check_term) {
        last_term_quorum_check.store(current_term);
        return true;
    }

    return last_check_term > 0; // We've validated some term
}

bool RaftStateMachine::has_config_quorum_check() const {
    ...
    size_t last_check = last_config_quorum_check.load();
    if (last_check >= required_quorum) {
        return true;
    }

    // If we're the leader and have committed entries, assume quorum
    if (is_leader()) {
...
bool RaftStateMachine::are_previous_ops_committed_in_current_config() const {
    // TLA+ OpCommittedInConfig pattern
    // Ensures that operations from previous configurations are committed

    // Constants for maximum gaps (can be made configurable)
    const int64_t MAX_UNCOMMITTED_GAP = 1000;
    const int64_t MAX_UNAPPLIED_GAP = 100;

    if (!node_manager) {
        return false;
    }

    // This would require RaftNodeManager to expose more status information
    // For now, simplified implementation
    return true; // Placeholder - needs proper implementation
}

bool RaftStateMachine::has_quorum_overlap(const raft_config::NodeConfiguration& new_config) const {
...
bool RaftStateMachine::validate_single_node_change(const raft_config::NodeConfiguration& new_config) const {
    std::shared_lock<std::shared_mutex> lock(current_config_mutex);
    return current_node_config.is_safe_single_node_change(new_config);
}

bool RaftStateMachine::add_node_safe(const std::string& node_to_add) {
    // Step 1: TLA+ ConfigIsSafe validation
    if (!config_is_safe()) {
        LOG(WARNING) << "Cannot add node - ConfigIsSafe validation failed";
        return false;
    }

    // Step 2: Create the new configuration
    raft_config::NodeConfiguration new_config;
    {
        std::shared_lock<std::shared_mutex> lock(current_config_mutex);
        new_config = current_node_config.create_single_node_change(
            node_to_add, "", 0); // TODO: Get current term
    }

    // Step 3: Validate single-node change safety
    if (!validate_single_node_change(new_config)) {
        LOG(WARNING) << "Cannot add node - single-node change validation failed";
        return false;
    }

    // Step 4: Validate new configuration can achieve quorum
    if (!has_quorum_overlap(new_config)) {
        LOG(WARNING) << "Cannot add node - quorum overlap validation failed";
        return false;
    }

    // Step 5: Apply the configuration change
    {
        std::unique_lock<std::shared_mutex> lock(current_config_mutex);
        current_node_config = new_config;
    }

    LOG(INFO) << "Node added safely: " << node_to_add;
    return true;
}

bool RaftStateMachine::remove_node_safe(const std::string& node_to_remove) {
    // Step 1: TLA+ ConfigIsSafe validation
    if (!config_is_safe()) {
        LOG(WARNING) << "Cannot remove node - ConfigIsSafe validation failed";
        return false;
    }
    ...
    raft_config::NodeConfiguration new_config;
    {
        std::shared_lock<std::shared_mutex> lock(current_config_mutex);
        new_config = current_node_config.create_single_node_change(
            "", node_to_remove, 0); // TODO: Get current term
    }

    // Step 3: Validate single-node change safety
    if (!validate_single_node_change(new_config)) {
        LOG(WARNING) << "Cannot remove node - single-node change validation failed";
        return false;
    }

    // Step 4: Validate new configuration can achieve quorum
    if (!has_quorum_overlap(new_config)) {
        LOG(WARNING) << "Cannot remove node - quorum overlap validation failed";
        return false;
    }
    ...
    LOG(INFO) << "Node removed safely: " << node_to_remove;
    return true;
}

void RaftStateMachine::handle_peer_failure(const braft::PeerId& failed_peer_id) {
    // Check if this failed peer corresponds to a hostname node
    std::shared_lock<std::shared_mutex> lock(current_config_mutex);
    for (const auto& hostname_node : current_node_config.hostname_nodes) {
        if (raft_config::peer_matches_hostname_node(failed_peer_id, hostname_node)) {
