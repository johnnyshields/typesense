...
public:
    ...
    /**
     * TLA+ ConfigIsSafe - Master safety validation
     * Comprehensive safety check combining all critical validations
     */
    bool config_is_safe() const;

    /**
     * TLA+ TermQuorumCheck - Leader authority validation
     * Ensures current leader has authority in the current term
     */
    bool has_term_quorum_check() const;

    /**
     * TLA+ ConfigQuorumCheck - Configuration acknowledgment
     * Ensures current configuration is acknowledged by a quorum
     */
    bool has_config_quorum_check() const;

    /**
     * TLA+ OpCommittedInConfig - Data consistency protection
     * Ensures previous operations are committed before config changes
     */
    bool are_previous_ops_committed_in_current_config() const;

    /**
     * TLA+ HasQuorumOverlap - Joint consensus validation
     * Validates that configuration changes maintain quorum overlap
     */
    bool has_quorum_overlap(const raft_config::NodeConfiguration& new_config) const;

    /**
     * TLA+ ValidateConfigChange - Single-node change safety
     * Validates that configuration change is a safe single-node modification
     */
    bool validate_single_node_change(const raft_config::NodeConfiguration& new_config) const;

    /**
     * Safe node addition with comprehensive TLA+ validation
     */
    bool add_node_safe(const std::string& node_to_add);

    /**
     * Safe node removal with comprehensive TLA+ validation
     */
    bool remove_node_safe(const std::string& node_to_remove);

    /**
     * Handle peer failure with DNS re-resolution trigger
     * Implements failure-triggered DNS refresh for 90% faster recovery
     */
    void handle_peer_failure(const braft::PeerId& failed_peer_id);

private:
    // Enhanced configuration management
    raft_config::NodeConfiguration current_node_config;
    std::atomic<bool> immediate_refresh_requested{false};
    std::atomic<uint64_t> last_term_quorum_check{0};
    std::atomic<size_t> last_config_quorum_check{0};
